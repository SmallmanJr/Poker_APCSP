...
def CheckWin(Cheating = False):
    if Cheating:
        ...
        for i, player in enumerate(activePlayers):
            ...
            IndexList.append(best_strength.entry.index)
            print(f"The best hand for {player} is {best_strength.entry.label}" )
            print("The index for the hand is ", best_strength.entry.index)
        print(f"the stregnth list is{strengthList}")

        
        winner = None

        LowestHand = min(IndexList)
        for t in strengthList:

            if t[1].entry.index > LowestHand:
                strongest_tuple = t
                winner = t[0]
                break
...
def ManageMoney(Winner):   
    for player in players:
        try:
            ...
            tempchip.betamount = 0 
            
            print(f"the new total {tempchip.total}, and the player bet is {tempchip.betamount}")
        except:
...
while not rl.window_should_close():
    ...
    if betting_mode:
        ...
        rl.draw_text(f"Total: {PlayerChips.total}", 100, 50, 30, rl.BLUE)
    ...
    if len(RiverHand.cards) == 5 and Turn == "Dealer" and Dealt:
        print ("the hand was",DealerHand)
        DrawDealerDamnit = True
